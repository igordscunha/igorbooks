...
const router = jsonServer.router('./backend/database.json')
let userdb = JSON.parse(fs.readFileSync('./backend/usuarios.json', 'UTF-8'))
...
server.post('/public/registrar', (req, res) => {
  ...
  fs.readFile("./backend/usuarios.json", (err, data) => {
    if (err) {
      const status = 401
      const message = err
      res.status(status).json({ status, message })
    ...
    fs.writeFile("./backend/usuarios.json", JSON.stringify(json), (err) => {
      if (err) {
        const status = 401
        const message = err
        res.status(status).json({ status, message })
...
server.post('/public/login', (req, res) => {
  ...
  if (!usuarioExiste({ email, senha })) {
    const status = 401
    const message = 'Nome, e-mail ou senha incorretos!'
    res.status(status).json({ status, message })
    return
  }
  ...
  const access_token = createToken({ tokenUsuario })
...
function extrairUsuarioDoToken(token) {
  try {
    ...
    return null; // Retorna nulo se o token for inválido ou estiver expirado
  }
}

server.get('/usuarios', (req, res) => {
  const token = req.headers.authorization?.split(' ')[1]; // Extrai o token do cabeçalho da requisição
  if (!token) {
    res.status(401).json({ message: 'Token de autenticação não fornecido' });
    return;
  }

  const usuario = extrairUsuarioDoToken(token);
  if (!usuario) {
